...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    setGenStatus('Extracting testable requirements...');
    try {
      const data = await api.generate(instructions);
      setGenStatus('Mapping tests to requirements...');
      await new Promise((r) => setTimeout(r, 300));
      setTestCases(data.testCases);
      setCoverage(data.coverage);
      setGenStatus(`${data.testCases.length} test cases ready across ${data.coverage.total} requirements.`);
      go('cases');
    } catch (err) {
      ...
      <div className="main">
        ...
        <div className="content">
          ...
          <section className={`view ${view === 'run' ? 'active' : ''}`}>
            ...
            <div className="card">
              <div className="step-label"><span className="step-num">3</span> Generate</div>
